...
public class addUserController implements Initializable {
	...
	public void addUser(ActionEvent event) throws Exception {
		 ...
		 || list.isDisable())
		{
			...
			else if(!list.isDisable() && checkStudent.isSelected())
			{
				errList.setText("You didn't selected the orientation for the student ");
			}
			else if(!list.isDisable() && checkTeacher.isSelected())
			{
		...
		else
		{
			...
			if(checkStudent.isSelected())
			{
				...
				Query q2 = em.createQuery("SELECT n FROM Usmjerenje n WHERE n.imeUsmjerenja = :a",Usmjerenje.class);
