...
public class deleteUserController implements Initializable {
	...
	public void testTeacher(MouseEvent event) throws IOException {
		if (checkTeacher.isSelected()) {
			...
			Query q = em.createQuery("SELECT p FROM Korisnik p WHERE p.isProdekan = :a and p.isNastavnik = :b",
					Korisnik.class);
	...
	public void deleteUser(ActionEvent event) throws Exception {
		if (!checkAll.isSelected() && !checkStudent.isSelected() && !checkTeacher.isSelected()) {
			...
			for (Korisnik e : temp) {
				...
				if (checkStudent.isSelected()) {
					Query q = em.createQuery("SELECT p FROM Student p WHERE " + "p.imeStud = :a and p.prezStud = :b",
							Student.class);
					...
					Query q = em.createQuery("SELECT p FROM Nastavnik p WHERE " + "p.imeNast = :a and p.prezNast = :b",
							Student.class);
					q.setParameter("a", e.getIme());
					q.setParameter("b", e.getPrezime());
					List<?> temp_list = q.getResultList();
					...
					if (temp_list.isEmpty()) {
						Query q1 = em.createQuery(
								"SELECT p FROM Nastavnik p WHERE " + "p.imeNast = :a and p.prezNast = :b",
								Student.class);
