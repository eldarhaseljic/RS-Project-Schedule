...
public class ReportController implements Initializable {
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		for(Cas c : casovi)
		{
			...
			String lokacija = c.getImeSale() + " " + c.getvrijemePocetkaCasaSat() + ":" + c.getVrijemePocetkaCasaMinuta() + "-" + c.getVrijemeZavrsetkaCasaSat() + ":" + c.getVrijemeZavrsetkaCasaMinuta();
			...
			if(dan.equals("Monday"))
			{
				...
				daysRange = Stream.iterate(startDate, date -> date.plusDays(1)).limit(numOfDays).filter( date -> date.getDayOfWeek()==DayOfWeek.MONDAY).collect(Collectors.toList());
			}
			
			else if(dan.equals("Tuesday"))
			{
				long numOfDays = ChronoUnit.DAYS.between(startDate, endDate);
				daysRange = Stream.iterate(startDate, date -> date.plusDays(1)).limit(numOfDays).filter( date -> date.getDayOfWeek()==DayOfWeek.TUESDAY).collect(Collectors.toList());
			}
			
			else if(dan.equals("Wednesday"))
			{
				long numOfDays = ChronoUnit.DAYS.between(startDate, endDate);
				daysRange = Stream.iterate(startDate, date -> date.plusDays(1)).limit(numOfDays).filter( date -> date.getDayOfWeek()==DayOfWeek.WEDNESDAY).collect(Collectors.toList());
			}
			
			else if(dan.equals("Thursday"))
			{
				long numOfDays = ChronoUnit.DAYS.between(startDate, endDate);
				daysRange = Stream.iterate(startDate, date -> date.plusDays(1)).limit(numOfDays).filter( date -> date.getDayOfWeek()==DayOfWeek.THURSDAY ).collect(Collectors.toList());
			}
			
			else if(dan.equals("Friday"))
			{
				long numOfDays = ChronoUnit.DAYS.between(startDate, endDate);
				daysRange = Stream.iterate(startDate, date -> date.plusDays(1)).limit(numOfDays).filter( date -> date.getDayOfWeek()==DayOfWeek.FRIDAY).collect(Collectors.toList());
			}
			
			else
			{
				long numOfDays = ChronoUnit.DAYS.between(startDate, endDate);
				daysRange = Stream.iterate(startDate, date -> date.plusDays(1)).limit(numOfDays).filter( date -> date.getDayOfWeek()==DayOfWeek.SATURDAY).collect(Collectors.toList());
			}
